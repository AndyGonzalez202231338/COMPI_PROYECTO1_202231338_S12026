...
class PkmParser {
            ...
            ParserPRO1.resultado.forEach { node ->
                when {
                    node is Array<*> && node.size == 3 && node[0] == "draw" -> {
                        ...
                        if (special == null) {
                            allErrors.add(ErrorToken(lexeme = nombre, line = 0, column = 0,
                                type = ErrorType.SEMANTICO,
    ...
    private fun convertirNodo(obj: Any?): FormElement? {
        return when (obj) {
            ...
            is DropQuestionNode -> FormElement.DropQuestion(
                ...
                correct = resolverNum(obj.correctVal)?.toInt() ?: -1,
                width = resolverNum(obj.width)?.toInt(), height = resolverNum(obj.height)?.toInt(),
                style = convertirEstilo(obj.style))
    ...
    private fun convertirSpecial(node: SpecialNode, args: List<Double>): FormElement? {
        ...
        return when (node.tipo) {
            ...
            "MULTIPLE" -> FormElement.MultipleQuestion(options = options, correct = emptyList(), width = resolvedWidth, height = resolvedHeight, style = convertirEstilo(node.style))
            else -> null
        }
    }
    ...
    private fun interpretarFor(node: ForNode): List<FormElement> {
        ...
        if (node.esRango) {
            ...
            if (tiposDecl.containsKey(node.variable) && tiposDecl[node.variable] != "number") return emptyList()
